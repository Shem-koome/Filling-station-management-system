...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_debt'])) {
    ...
    if ($debtor_name && $description && $amount > 0) {
        $stmt = $conn->prepare("INSERT INTO debts (debtor_name, description, total_amount, remaining_amount, status) VALUES (?, ?, ?, ?, 'pending')");
        $stmt->bind_param("ssdd", $debtor_name, $description, $amount, $amount);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pay_debt'])) {
    ...
    if ($debt && $payment > 0 && $payment <= $debt['remaining_amount']) {
        ...
        // Update debts remaining and status if cleared
...
<div class="main-content">
    ...
    <form method="POST" style="margin-bottom: 30px;">
        ...
        <button type="submit" name="new_debt">Add Debt</button>
    </form>
